...
class FrameRestorer:
    ...
    def _restore_clip(self, clip: Clip):
        ...
        if self.re_mosaic_block_size is not None and self.re_mosaic_block_size >= 2:
            for i in range(len(clip.frames)):
                ...
                # Convert back to tensor preserving device and dtype
    ...
    def _fast_block_mosaic(self, img, mask, n):
        ...
        import cv2
        h, w = img.shape[:2]
        # Create 3D mask for blending (0 or 1)
        ...
        # Ensure blending uses floats
        result = img.astype(np.float32) * (1.0 - mask_3d.astype(np.float32)) + blocky_img.astype(np.float32) * mask_3d.astype(np.float32)
